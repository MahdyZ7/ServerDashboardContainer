...
def generate_parsers(schema: Dict[str, Any]) -> List[str]:
    ...
    base_path = Path(__file__).parent.parent.parent / 'srcs' / 'DataCollection' / 'generated'
...
""")
        ...
        for field in bash_fields:
            ...
            if bash_format == 'raw':
                parsing_code.append(f"    result['{name}'] = parts[{idx}].strip() if len(parts) > {idx} else None\n")
