...
def parse_server_metrics(bash_output: str) -> dict:
    ...
    if len(parts) > 4:
        result["ram_used"] = (
            parts[4].split("/")[0].strip() if "/" in parts[4] else parts[4].strip()
        )
    if len(parts) > 4:
        result["ram_total"] = (
            parts[4].split("/")[1].strip() if "/" in parts[4] else parts[4].strip()
        )
    result["ram_percentage"] = parts[5].strip() if len(parts) > 5 else None
    if len(parts) > 6:
        result["disk_used"] = (
            parts[6].split("/")[0].strip() if "/" in parts[6] else parts[6].strip()
        )
    if len(parts) > 6:
        result["disk_total"] = (
            parts[6].split("/")[1].strip() if "/" in parts[6] else parts[6].strip()
        )
    if len(parts) > 7:
